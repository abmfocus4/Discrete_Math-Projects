...
Set::Set(const int numElements, const int data[]) {
	...
	if(_numElements > 0)
	{
		...
		int copy_data[_numElements];
		int count =0;
		
		//creating another copy of the given tuple
		for(int i=0; i<_numElements; i++)
			copy_data[i] = data[i];
		...
		int req_data_set[size_distinct];
		_pTuples = new Tuple[size_distinct];
		
		for(int m=0; m<size_distinct; m++)
			req_data_set[m] = data_distinct[m];
		
		//creating a set that contains only distinct elements
		for(int i=0; i++; i<_numElements)
			_pTuples[i] = req_data_set[i];
	}
...
Set::Set(const int numElements, const Tuple tuples[]) {
	...
	if(numElements >0)
	{
		...
		Tuple copy_tuples[numElements];
		int count =0;
		
		//creating another copy of the given tuple
		for(int i=0; i<numElements; i++)
			copy_tuples[i] = tuples[i];
		...
		for(int i=0; i<numElements;i++)
		{
			...
			{
				for(int l=0; l<numElements; l++){
					if(tuples[i] == copy_tuples[j] && count == 0)
					{
